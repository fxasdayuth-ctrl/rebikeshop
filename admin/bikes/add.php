...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($customer_id) || empty($bike_type_id) || empty($brand)) {
        ...
        try {
            $stmt = $conn->prepare("
                INSERT INTO bikes (customer_id, bike_type_id, brand, model, color, serial_number, purchase_date, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
            ");
            if (!$stmt) {
                throw new Exception("ข้อผิดพลาดในการเตรียมคำสั่ง SQL: " . $conn->error);
            }
            $stmt->bind_param("iisssss", $customer_id, $bike_type_id, $brand, $model, $color, $serial_number, $purchase_date);
            
            if ($stmt->execute()) {
                $success = 'เพิ่มจักรยานสำเร็จ!';
                // รีเซ็ตฟอร์ม
                $customer_id = $bike_type_id = $brand = $model = $color = $serial_number = $purchase_date = '';
            } else {
                $error = 'เกิดข้อผิดพลาดในการเพิ่มจักรยาน: ' . $stmt->error;
            }
            $stmt->close();
        } catch (Exception $e) {
...
<head>
    ...
    <title>เพิ่มจักรยาน - RebikeShop</title>
...
<body>
    <div class="container">
        <div class="form-container">
            <h2 class="mb-4">เพิ่มจักรยานใหม่</h2>
            ...
            <form method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="bike_type_id" name="bike_type_id" required>
                        ...
                        <?php foreach ($bike_types as $bike_type): ?>
                            <option value="<?= $bike_type['id'] ?>" <?= isset($bike_type_id) && $bike_type_id == $bike_type['id'] ? 'selected' : '' ?>>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="model" name="model" value="<?= htmlspecialchars($model ?? '') ?>">
                </div>
                <div class="mb-3">
                    <label for="color" class="form-label">สี</label>
                    <input type="text" class="form-control" id="color" name="color" value="<?= htmlspecialchars($color ?? '') ?>">
                </div>
                <div class="mb-3">
                    <label for="serial_number" class="form-label">หมายเลขซีเรียล</label>
                    <input type="text" class="form-control" id="serial_number" name="serial_number" value="<?= htmlspecialchars($serial_number ?? '') ?>">
                </div>
                <div class="mb-3">
                    <label for="purchase_date" class="form-label">วันที่ซื้อ</label>
                    <input type="date" class="form-control" id="purchase_date" name="purchase_date" value="<?= htmlspecialchars($purchase_date ?? '') ?>">
                </div>
