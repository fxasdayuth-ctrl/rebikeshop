...
$work = [
    ...
    'model' => '',
    'color' => '',
    'name' => '', // Still using 'name' for database consistency
    'description' => '', // Still using 'description' for database consistency
    'total_cost' => '' // New field for total_cost
];
$customer_id = null;
$bike_id = null;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $work['total_cost'] = trim($_POST['total_cost'] ?? ''); // New field
    ...
    if (!empty($work['total_cost']) && !is_numeric($work['total_cost'])) {
        $errors['total_cost'] = 'ราคาค่าซ่อมต้องเป็นตัวเลข';
    } elseif (!empty($work['total_cost']) && $work['total_cost'] < 0) {
    ...
    if (empty($errors)) {
        try {
            ...
            // เพิ่มงานซ่อม
            $stmt = $pdo->prepare("INSERT INTO repair_orders (customer_id, bike_id, name, description, total_cost, created_at) VALUES (?, ?, ?, ?, ?, NOW())");
            $total_cost = !empty($work['total_cost']) ? $work['total_cost'] : null;
            $stmt->execute([
                $customer_id,
                $bike_id,
                $work['name'],
            ...
            $pdo->commit();
            $success = 'เพิ่มงานและลูกค้าใหม่สำเร็จ!';
            $work = [
                'first_name' => '',
                'last_name' => '',
                'phone' => '',
            ...
            <div class="col-12">
                ...
                <form method="POST" action="">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="notes" name="notes"><?php echo htmlspecialchars($work['notes']); ?></textarea>
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="total_cost" class="form-label">ราคาค่าซ่อม (บาท)</label>
                        <input type="number" step="0.01" min="0" class="form-control" id="total_cost" name="total_cost" value="<?php echo htmlspecialchars($work['total_cost']); ?>">
                        <?php if (isset($errors['total_cost'])): ?><div class="text-danger"><?php echo $errors['total_cost']; ?></div><?php endif; ?>
                    </div>
